...
- You MUST check that partition counts per broker are within the recommended limit for the broker instance type before upgrading. Consult the [Amazon MSK best practices — right-size your cluster](https://docs.aws.amazon.com/msk/latest/developerguide/bestpractices.html#bestpractices-right-size-cluster) documentation for supported partition counts per broker size.

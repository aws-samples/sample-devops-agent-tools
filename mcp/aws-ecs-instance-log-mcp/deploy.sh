...
STACK_NAME="${1:-EcsInstanceLogMcpStack}"
...
if [ -n "$ECS_INSTANCE_ROLE_ARNS" ]; then
  ...
  if [ ${#CLUSTER_LIST[@]} -eq 0 ]; then
    echo "WARNING: No ECS clusters found in the selected region(s)."
    echo ""
    ...
    if [ ${#SELECTED_CLUSTERS[@]} -eq 0 ]; then
      echo "No valid clusters selected. Skipping instance role detection."
    else
      echo ""
